    ...
    return (
        <PhoneContainer>
            ...
            >
                ...
                >
                    <AvatarsDuoSuccess avatarSeedOne={pairData?.myAvatarSeed ?? ""} avatarSeedTwo={pairData?.partnerAvatarSeed ?? ""} />
                    <Typography variant="headingExtraLarge">{`You have successfully paired up with ${pairData?.partnerUsername}!`}</Typography>
                    <Typography variant="bodyMedium">Everything is set up — it’s time to make history!</Typography>
                </Stack>
            </Stack>
            <Box sx={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "16px" }}>
                <NBButton onClick={() => handleNavigationTransition("/home")}>Let's go!</NBButton>
            </Box>
        </PhoneContainer>
    );
};
