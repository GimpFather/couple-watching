...
import {
	deletePair,
	getMyPairWithProfiles,
	handleJoinPairByCode,
	handlePairByYourself,
} from "~/api/services/pairs.service";
...
export const useHandleJoinPairByCode = () => {
	...
	return useMutation({
		...
		mutationFn: ({
			...
			partnerCode: string;
			myProfileId: string;
		}) => handleJoinPairByCode(partnerCode, myProfileId),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: [QUERY_KEYS.GET_YOUR_PAIR_DATA],
			});
...
export const useHandlePairByYourself = () => {
	const queryClient = useQueryClient();
	return useMutation({
		mutationKey: [QUERY_KEYS.HANDLE_PAIR_BY_YOURSELF],
		mutationFn: (data: MakePairByYourselfData) => handlePairByYourself(data),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: [QUERY_KEYS.GET_YOUR_PAIR_DATA],
			});
